...
const ParaNode = ({ id, data, selected }) => {
  ...
  // Update position in localStorage when it changes
  useEffect(() => {
    const uniqueKey = getUniqueKey(id);
  ...
  return (
    <div style={{ position: "relative", display: "inline-block", zIndex: 1 }}>
      ...
      >
        <path
          d={`M${size.width * 0.083} ${size.height * 0.833} 
              L${size.width * 0.833} ${size.height * 0.833} 
              L${size.width} ${size.height * 0.167} 
              L${size.width * 0.25} ${size.height * 0.167} Z`}
          fill={bgColor}
          stroke={borderColor}
